...
class SiteScroll {
  ...
  init() {
    ...
    // this.scroll.on("loop", this._onLoop);
  ...
  destroy() {
    if (this.scroll) {
      ...
      // this.scroll.off("loop", this._onLoop);
      this.scroll.destroy();
    }
  ...
  start() {
    ...
    if( !this.hasScrollCallEvent ) {
      this.hasScrollCallEvent = true;
  ...
  // _onLoop() {
  //   console.log('loop');
  //   // this.emitter.emit(`${this.name}.loop`);
  // }
